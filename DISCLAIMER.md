...
## Provisionamento real de recursos (atenção especial)

Esta skill pode, quando usada em modo de deploy automático, **provisionar recursos reais
no Azure** na assinatura e tenant informados, o que **pode gerar custos** e alterar o
ambiente. Você é o único responsável por:

- Confirmar a assinatura/tenant corretos antes de qualquer deploy.
- Usar assinaturas de sandbox/laboratório, nunca produção.
- Revisar o `what-if`/`plan` e o preflight antes de aprovar o deploy.
- **Rodar o teardown** ao final para evitar custos residuais.
...
- Falhas operacionais
- Impactos financeiros (incluindo custos de recursos provisionados)
...
- Garantir que há rollback possível (teardown)
